...
export function evaluateMatcher(matcher: IntentMatcher, message: string): boolean {
  switch (matcher.type) {
    ...
    case 'keyword': {
      if (!matcher.keywords || matcher.keywords.length === 0) return false
    ...
    case 'length': {
      ...
      if (matcher.minLength !== undefined && len < matcher.minLength) return false
      if (matcher.maxLength !== undefined && len > matcher.maxLength) return false
      return true
    }
...
function deriveConfidence(matchers: IntentMatcher[]): 'high' | 'medium' | 'low' {
  ...
  for (const m of matchers) {
    if (m.type === 'regex') hasRegex = true
    if (m.type === 'keyword') hasKeyword = true
  }

  if (hasRegex) return 'high'
...
export const DEFAULT_INTENT_RULES: IntentRule[] = [
  ...
  // -----------------------------------------------------------------------
  {
    ...
    matchers: [{
      type: 'regex',
      pattern: '\\b(?:build|create|implement|design|write|develop)\\b.{5,80}\\b(?:and|then)\\b.{5,80}\\b(?:build|create|implement|design|write|develop|test|review|deploy)\\b',
      flags: 'i',
    }],
  },
